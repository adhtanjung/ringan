		...
		if (USE_SUPABASE_FOR.includes(selectedDataType.value)) {
			...
			const fileData = await new Promise((resolve, reject) => {
				...
				if (file.name.endsWith(".json")) {
					...
					// For now, only JSON is supported for Supabase direct import in this simple implementation
					reject(
						new Error(
							"Only JSON files are supported for direct Supabase import in this version. For CSV/Excel, please convert to JSON or use the legacy API."
						)
					);
				}
			});
