...
const acceptedFileTypesText = computed(() => {
	return "CSV, JSON files up to 10MB";
});
...
const validateAndSetFile = async (file) => {
	...
	if (!isValidType) {
		errorMessage.value = "Please select a valid CSV or JSON file.";
		return;
	}
		...
		if (USE_SUPABASE_FOR.includes(selectedDataType.value)) {
			...
			if (selectedDataType.value === "problem_types") {
				...
				for (const item of items) {
					// Check if category_id is missing or not in human-readable format
				...
				if (autoGeneratedCount > 0) {
					console.log(`Auto-generated ${autoGeneratedCount} category IDs`);
				}
			}
			...
			// Batch insert
			// Note: we remove 'id' from items if they are from MongoDB (since they are strings, but Supabase might use UUID)
			// or we keep them if the schema allows text IDs.
			...
			const successMsg =
				selectedDataType.value === "problem_types"
					? `Successfully imported ${cleanedItems.length} records into Supabase.`
					: `Successfully imported ${cleanedItems.length} records into Supabase.`;
