			...
			<div class="border-b bg-background px-6 py-5">
				<DialogHeader class="space-y-1">
					<DialogTitle class="text-xl font-semibold">
						{{ isEditing ? "Edit" : "Create" }} Opening Question
					</DialogTitle>
					<DialogDescription class="text-sm leading-6">
						Define a top-of-funnel question shown before general questions. Routes to another opening question or a category.
					</DialogDescription>
				</DialogHeader>
			</div>
					...
					<div class="space-y-6 px-6 py-6">
						...
						<section class="space-y-2">
							...
							<Textarea
								...
								rows="3"
								placeholder="e.g. Is this your first time seeking mental health support?"
						...
						>
							...
							<div class="space-y-2">
								...
								<div class="grid gap-3 sm:grid-cols-[160px_1fr]">
									...
									>
										...
										<SelectContent>
											<SelectItem value="opening_question">Opening Question</SelectItem>
											<SelectItem value="category">Category</SelectItem>
										</SelectContent>
									</Select>
									<!-- Opening question dropdown -->
									<Select
										v-if="formData.yes_destination_type === 'opening_question'"
									...
									>
										...
										<SelectContent>
											<SelectItem v-for="q in openingQuestions" :key="q.id" :value="q.id">
												{{ q.label }}
											</SelectItem>
										</SelectContent>
									</Select>
									<!-- Category dropdown -->
									<Select
										v-else-if="formData.yes_destination_type === 'category'"
										:model-value="formData.yes_destination_value ?? ''"
										@update:model-value="(v) => (formData.yes_destination_value = v || null)"
									>
										<SelectTrigger class="h-10 text-sm">
							...
							<div class="space-y-2">
								...
								<div class="grid gap-3 sm:grid-cols-[160px_1fr]">
									...
									<!-- Opening question dropdown -->
									<Select
										v-if="formData.no_destination_type === 'opening_question'"
									...
									<!-- Category dropdown -->
									<Select
										v-else-if="formData.no_destination_type === 'category'"
										:model-value="formData.no_destination_value ?? ''"
										@update:model-value="(v) => (formData.no_destination_value = v || null)"
									>
										<SelectTrigger class="h-10 text-sm">
						...
						>
							<div class="flex items-start justify-between gap-3">
								<div class="space-y-1">
									...
									<p class="text-xs text-muted-foreground">
										Each choice routes to another opening question or a category.
									</p>
								</div>
								<Button
									type="button"
									...
									<div class="space-y-3">
										...
										<div class="space-y-1.5">
											...
											<Input
												...
												placeholder="e.g. I need immediate help"
												class="h-10 text-sm"
											/>
										</div>
										...
										<div class="grid gap-3 sm:grid-cols-[160px_1fr]">
											...
											<div class="space-y-1.5">
												...
												<!-- Opening question dropdown -->
												<Select
													v-if="choice.destination_type === 'opening_question'"
												...
												<!-- Category dropdown -->
												<Select
													v-else-if="choice.destination_type === 'category'"
													:model-value="choice.destination_value"
													@update:model-value="(v) => (choice.destination_value = v)"
												>
													<SelectTrigger :id="`choice_dest_val_${index}`" class="h-10 text-sm">
						...
						<section class="space-y-3">
							...
							>
								<div class="space-y-2">
									...
									/>
									<p class="text-xs text-muted-foreground">
										Position of this question in the opening sequence.
									</p>
								</div>
							</div>
						</section>
...
interface Choice {
	option_text: string;
	destination_type: "opening_question" | "category" | "";
...
const props = defineProps<{
	isOpen: boolean;
	item: OpeningQuestion | null;
}>();

const emit = defineEmits<{
	close: [];
	save: [data: OpeningQuestion];
}>();

const getInitialState = (): OpeningQuestion => ({
	question_id: "",
	question_text: "",
	response_type: "",
...
const formData = reactive<OpeningQuestion>(getInitialState());
...
const openingQuestions = ref<QuestionOption[]>([]);
...
const fetchLookups = async () => {
	...
	try {
		...
		openingQuestions.value = (qRes || []).map((r: any) => ({
			id: r.question_id,
			label: r.question_text ? `${r.question_id} – ${r.question_text}` : r.question_id,
		}));
	} catch (e) {
		console.error("OpeningQuestionModal: failed to fetch lookups", e);
	} finally {
...
const generateQuestionId = async () => {
	try {
		...
		if (data && data.length > 0) {
			...
			if (!isNaN(num)) {
				return `OQ${String(num + 1).padStart(3, "0")}`;
			}
		}
		return "OQ001";
	} catch {
...
const destinationLabel = (type: string) => {
	if (type === "opening_question") return "Opening Question";
