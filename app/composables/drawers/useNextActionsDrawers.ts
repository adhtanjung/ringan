...
interface UseNextActionsDrawersOptions {
	supabase: SupabaseClient<any, any, any>;
	onEdit: (item: Record<string, unknown>) => void;
}

export const useNextActionsDrawers = ({
	supabase,
	onEdit,
}: UseNextActionsDrawersOptions) => {
	...
	const nextActionContext = ref(createRelationSummaryState());
	const listPrefetchCache = ref<Record<string, LinkedRecordSection[]>>({});

	const actionIdSpec = getRelationSpec("next_actions_detail", "action_id");
	const actionRecordIdSpec = getRelationSpec("next_actions_detail", "id");
	...
	const loadNextActionContextSummary = async (
		rawId: unknown = viewingItem.value?.action_id,
	) => {
		const relationId = normalizeRelationId(rawId);
		const relationKey = actionIdSpec.relationKey;
		const idLabel = actionIdSpec.label;

		if (!relationId) {
			nextActionContext.value = {
				status: "missing",
				loading: false,
				error: "",
				data: null,
		...
		try {
			...
			console.error("Error prefetching next-action relation context:", error);
			nextActionContext.value = {
				status: "error",
				loading: false,
				error: "We couldn't resolve linked feedback prompts. Please try again.",
				data: {
					title: "Linked relation unavailable",
	...
	const openDetailView = (item: Record<string, unknown>) => {
		...
		void loadNextActionContextSummary(item?.action_id);
	};
	...
	const closeDetailSheet = () => {
		...
		setTimeout(() => {
			...
			showTechnicalDetails.value = false;
			nextActionContext.value = createRelationSummaryState();
		}, 300);
	};
