...
export const FORM_SCHEMAS: Record<string, FormFieldConfig[]> = {
	...
	suggestions: [
		{
			key: "suggestion_id",
			label: "Suggestion ID (Auto)",
			type: "text",
			required: true,
			placeholder: "Generated automatically (e.g., S_0001)",
		},
		{
			key: "sub_category_id",
			label: "Linked Subcategory",
			type: "text",
			required: true,
	...
	next_actions: [
		{
			key: "action_id",
			label: "Action ID (Auto)",
			type: "text",
			required: true,
			placeholder: "Generated automatically (e.g., A_0001)",
		},
		{
			key: "action_text",
			label: "Action Text",
			...
			required: true,
			rows: 3,
			placeholder: "e.g., Acknowledge concern and continue check-in flow",
			fullWidth: true,
		},
		// Temporarily hidden until next-action execution logic uses them again.
		// {
		// 	key: "parameters",
		// 	label: "Parameters",
		// 	type: "json",
